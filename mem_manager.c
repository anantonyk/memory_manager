...
size_t process_memory_size = 100000000;
void* begin_ptr;
free_block* first_free_block;

void init_malloc() {
	begin_ptr = malloc(process_memory_size);
	process_memory_init = true;
	first_free_block = (free_block*)begin_ptr;
	first_free_block->next = END_LL_SEQ;
}
...
bool validate_empty_block(void*  ptr, size_t size) {
	...
	if (ptr + size >= begin_ptr + process_memory_size)
		return false;
...
int size_of_free_block(free_block * fb) {
	int size = 0;
	void* tmp = (void*) fb;
	while (((start_of_block*)tmp)->block_ident != END_LL_SEQ) {
...
void init_allocated_block(void* ptr, size_t block_size) {
	...
	start->block_ident = START_SEQ;
	start->size = block_size - sizeof(start_of_block);
}
...
void* my_malloc(size_t size) {
	...
	if (first_free_block == END_LL_SEQ)
		return NULL;
	
	//if it is one free block
	if (first_free_block->next == END_LL_SEQ) {
	...
	while (tmp_free_block->next != END_LL_SEQ){
...
void my_free(void* ptr) {
	...
	if (ptr >= begin_ptr + process_memory_size)
		return;
	...
	if (new_free_block < first_free_block) {
		...
		//new_free_block->next->next = first_free_block->next; IS IT NEEDED???
